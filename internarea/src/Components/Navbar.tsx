...
const Navbar = () => {
  ...
  const handleLangChange = async (code: string) => {
    ...
    if (!emailToUse) {
      const entered = window.prompt("Enter your email to verify language change:");
      if (!entered) return;
      emailToUse = entered;
    }
          ...
          {LANGUAGES.map((l) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center justify-between ${
                lang === l.code ? "text-blue-600 font-semibold" : "text-gray-700"
              }`}
            >
              {l.label}
                      ...
                      <div className="absolute top-12 right-0 bg-white border border-gray-200 rounded-xl shadow-lg p-6 w-80 z-50">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between text-xs text-gray-500">
                            ...
                            <button onClick={() => setshowEmailLogin(false)} className="hover:text-gray-700">
                              {t("cancel")}
                            </button>
                          </div>
                        </div>
